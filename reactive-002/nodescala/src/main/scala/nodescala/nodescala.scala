...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val serverLoopSubscription = Future.run() { ct =>
      Future {
        def handle(listener: Listener, handler: Request => Response): Unit = {
          val nextReq = listener.nextRequest()
          nextReq onComplete { _ => if (ct.nonCancelled) handle(listener, handler) }
          nextReq onSuccess {
